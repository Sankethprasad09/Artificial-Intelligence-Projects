...
class WumpusWorld:
    ...
    - The agent can detect stench, breeze, glitter, bump, and a persistent scream (if the wumpus is dead).
    ...
    def percept(self, location):
        ...
        Each of these elements can be None or a string (e.g. 'Stench').
        """
        if location is None:
        ...
        # Stench: if location == wumpus_location or adjacent to wumpus
        # Wumpus can be dead or alive; it always stinks.
    ...
    def climbed(self):
        """
        Climb out of the cave if at the exit location (1,1).
        This removes the agent from the cave (agent_location=None).
        """
    ...
    def shot(self):
        ...
        if not self.agent_alive or not self.wumpus_alive:
            return  # Either the agent is dead or wumpus already dead, no effect.

        ax, ay = self.agent_location
        wx, wy = self.wumpus_location if self.wumpus_location else (None, None)

        if self.agent_direction == 'East':
            # Wumpus is east if same Y, but wumpus X > agent X
            if wy == ay and wx > ax:
    ...
    def adjacent(self, location, target):
        """
        Are location and target directly north, south, east, or west of each other?
        """
    ...
    def agent_bumped_wall(self):
        """
        Has the agent bumped into a wall? Equivalently, is the agent facing
        a boundary square's outside edge?
        """
        if not self.agent_location:
            return False  # Agent is out of the cave
        x, y = self.agent_location
    ...
    def wumpus_east_of_agent(self):
        """
        Wumpus is east of the agent if they share the same y,
        and the wumpus's x > agent's x.
        """
